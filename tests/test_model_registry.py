...
def test_legacy_backend_flags_apply_only_to_the_legacy_model(registry, tmp_path):
    legacy = gguf(tmp_path / "legacy.gguf")
    flags = ["--reasoning-format", "deepseek", "--reasoning-preserve"]
    assert launch_command(registry, "code", Path("/server"), str(legacy), "legacy", flags)[-3:] == flags
    registry.select("code", Selection(model_id="base"), expected_revision=registry.revision())
    assert "--reasoning-preserve" not in launch_command(registry, "code", Path("/server"), str(legacy), "legacy", flags)
