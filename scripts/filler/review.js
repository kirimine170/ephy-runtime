...
function stop(reason = 'cancel') {
  ...
  if (trial) {
    ...
    clearTimeout(t.timer); t.controller?.cancel(reason); t.monitor?.stop();
  ...
  try {
    ...
    if ($('microphone').checked) {
      t.monitor = await startFillerBargeIn({context, mediaDevices: navigator.mediaDevices, isCurrent: current,
        onSpeech: () => stop('barge_in'), onUnavailable: () => stop('invalidated')});
...
try {
  ...
  for (const row of rows) {
    ...
    const name = document.createElement('strong'); name.textContent = row.phrase;
