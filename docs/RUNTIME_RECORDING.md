...
user finalをprivate queueへ保全してからLLMを起動する．保全に失敗した発言ではLLMを開始せずエラーを表示する．text／RAGのstreamも文・Markdownの確定境界でcheckpointし，OFFや停止前の接頭部分を保持する．assistant保全に失敗した場合もturnを成功扱いにせず通知する．再起動時は未完了assistantを保全済みの確定本文と生成・再生の観測から閉じ，未観測の完了を推定しない．同じcandidateと署名済みbytesを再送し，Karte停止・Runtime強制終了・保存後receipt前の停止を跨いで同じ保存結果へ収束させる．
