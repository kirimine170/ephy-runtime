...
このファイルは`c509dfc`から保存した**2026-09-13の旧実験結果**である．下記測定を2026-09-14の常駐worktreeで再実行した結果として扱わない．常駐版の新規測定は[resident-results.md](resident-results.md)，現在の実行方法は[README](README.md)を参照する．旧別画面と旧launcherは常駐版に取り込まない．
