...
Developer Modeでは，明示設定した場合に限り，project，分類，類似文書確認，placement，review hashがすべて確定したpublish可能な候補をEphyからKarte pending outboxへ自動送信できる．分類不能，Context未確認，stale planは従来どおり停止し，Karte側のcanonical Markdownへの採用／破棄は人間のreview境界として残す．Project，kind，解決方法，append先を変更したカードは自動で再計画され，古いreview hashのまま送信できない．

自動送信を有効にした場合，Project，kind，解決方法，append先を変更したカードは再計画後にも同じpublish条件を評価し，条件を満たしたときだけpending outboxへ送信する．
...
Launcherは`.karte-data-dir`へ単一行の既存絶対directoryだけを書き，直後に同じ値をread-backして検証する．これにより，bundleを手動再起動した際にContext processorが誤ったrootで停止する状態を起動前に検出する．
