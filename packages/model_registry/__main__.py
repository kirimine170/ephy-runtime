...
def launch_command(registry, role, server, fallback_path, fallback_alias, fallback_args=()):
...
def main():
    ...
    launch.add_argument("--fallback-args", nargs=argparse.REMAINDER, default=[])
    ...
    try:
        if args.command == "list":
            ...
            command = launch_command(registry, args.role, args.server, args.fallback_path, args.fallback_alias, args.fallback_args)
            if args.dry_run:
                result = {"argv": command}
            else:
